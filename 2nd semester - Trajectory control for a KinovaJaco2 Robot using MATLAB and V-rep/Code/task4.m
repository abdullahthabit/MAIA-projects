...
function [t, q, q_act] = main_template
    ...
    if strcmp(algorithm,'inverse')
        K = 1* diag([20 20 20 20 20 20]);
        fprintf('\n algoritmo con la trasposta dello jacobiano')
    else
        K = 0.1*diag([90 90 90 90 90 90]);
    ...
    for i=1:N
        ...
        % task 4
        k_a = 500;
        dq_a = k_a*( manipulability_Jacobian(DH))';
        mpMes(:,i) = manipulability_measure(DH);
        I_m = eye(size(J,2));
        compensate(:,i) = ((I_m - pinv(J)*J)*dq_a);
        
        if strcmp(algorithm,'transpose')
            dq(:,i) = J'*(K*error(:,i)+v_d) + (I_m - J'*J)*dq_a;
        else
            dq(:,i) = (pinv(J)*(K*error(:,i)+v_d)) + ((I_m - pinv(J)*J)*dq_a);
        end
        % integration
        if i<N
            q(:,i+1) = q(:,i) + Ts*dq(:,i);
    ...
    figure;
    plot(t,compensate)
    title('singulatrity compensation')
    xlabel('time [s]')
