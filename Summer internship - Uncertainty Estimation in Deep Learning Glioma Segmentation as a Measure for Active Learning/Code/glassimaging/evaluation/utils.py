...
def plotResultImage(datamanager, resultpath, uncertaintypaths, savepath, subject, output_type='save'):
    basefile = datamanager.getFileName(subject, 't1')
    segfile = datamanager.getFileName(subject, 'seg')
    if output_type == 'save':
        figure, (axes1, axes2, axes3) = plt.subplots(3, 1, figsize=(10, 10))
        cut = plotting.find_xyz_cut_coords(segfile, activation_threshold=0.5)
        plotting.plot_roi(segfile, basefile, cut_coords=cut, axes=axes1, title='Target')
        plotting.plot_roi(resultpath, basefile, cut_coords=cut, axes=axes2, title='Result')
        plotting.plot_roi(uncertaintypaths, basefile, cut_coords=cut, axes=axes3, title='uncertainty')
