...
class StandardEvaluator:
    ...
    def inferWithImage(self, image, uncertainty=False):
        if uncertainty:
    ...
    def predictive_entropy(self, prob):
        return -1 * torch.sum(torch.log(prob) * prob, dim=0)

    def applyFunc(self, func, M):
    ...
    def segmentWholeArray(self, input_array, inputsize, targetsize):
        ...
        epistemicOutput = np.zeros(input_array.shape[0:1] + input_array.shape[2:5])
        ...
        for start, end in zip(startlocations, endlocations):
            ...
            t = 100
            monteCarlo_output = torch.zeros(t, 5, 128, 128, 128, dtype=torch.float32, device='cuda')
            for i in range(t):
                outputpatch = self.inferWithImage(inputpatch, uncertainty=True)
                monteCarlo_output[i, :, :, :, :] = outputpatch
            outputpatch = torch.mean(monteCarlo_output, dim=0)
            outputpatch = torch.unsqueeze(outputpatch, dim=0)
            epistemicUncertainty = self.applyFunc(self.predictive_entropy, outputpatch)
            epistemicUncertainty = epistemicUncertainty.cpu().numpy()
            ...
            if outputpatch.shape != tuple(targetsize):
                ...
                epistemicUncertainty = epistemicUncertainty[:, crop[0]:-crop[0], crop[1]:-crop[1], crop[2]:-crop[2]]
            ...
            epistemicOutput[slice_obj] = epistemicUncertainty

            del monteCarlo_output
        return output, epistemicOutput
    ...
    def segmentNifti(self, images, header_files, inputsize, savepaths, uncertaintypaths, crop=16):
        input_array = np.stack(images)
        n_classes = self.getNumClasses()
        targetsize = [i - crop * 2 for i in inputsize]
        output, epistemicOutput = self.segmentWholeArray(input_array, inputsize, targetsize)
        for i in range(0, len(savepaths)):
            ...
            epistimic_img = nib.Nifti1Image(epistemicOutput[i], ff, hdr)
            nib.save(epistimic_img, uncertaintypaths[i])
        return output, epistemicOutput
