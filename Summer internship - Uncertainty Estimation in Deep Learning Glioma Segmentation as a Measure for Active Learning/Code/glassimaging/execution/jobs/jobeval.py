...
class JobEval(Job):
    ...
    def run(self):
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            classifications, epistemicUncertainty = evaluator.segmentNifti(images, segfiles, patchsize, resultpaths,
                                                                           uncertaintypaths)
            import pickle
            segmentations = segs.numpy()
            picklepath = myconfig["pickleVar"]
            with open(picklepath, 'wb') as f:
                pickle.dump([segmentations, classifications, epistemicUncertainty, segfiles, subjects], f)
...
if __name__ == '__main__':
    ...
    # tmpdir = "./../../../experiments/"
    # job = JobEval(configfile, name, tmpdir)
    # job.run()
